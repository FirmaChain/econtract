...
export default class extends React.Component {
	...
	componentDidMount(){
        ...
        this.unblock = history.block(targetLocation => {
            if(this.blockFlag){
                if(window._confirm("계약 배포를 중단하고 현재 페이지를 나가시겠습니까?"))
                    return true;
                else
                    return false;
            }
    ...
    onClickNext = async ()=>{
        ...
        await window.showIndicator()
        let resp = await this.props.new_contract( subject, imgs, (counterparties || []).map(e=>e.code), [this.props.user_info.publickey_contract].concat((counterparties || []).map(e=>e.code)) );
        if(resp){
            //this.unblock();
